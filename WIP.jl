...
# ╔═╡ a7f3d653-d02b-4e0e-8fcc-247b9db9d275
md"""
...
"Given some IR generates a MethodInstance suitable for passing to infer_ir!, if you don't already have one with the right argument types. Uses the ir.argtypes to determine level of specializion (not JIT)"
...
md"""
## Lets make a macro for running the compiler manually.
"""
...
# ╔═╡ 489846ad-6d87-4fd6-b800-87e3da18b67d
ir.cfg.blocks

# ╔═╡ 2973db96-469b-4ca0-906b-dc3d96b61acb
function find_loops(blocks)
	loopbacks = CC.BasicBlock[]
	for (cur_ii, block) in enumerate(blocks)
		if any(<=(cur_ii), block.succs)
			push!(loopbacks, block)
		end
	end
	return loopbacks
end

# ╔═╡ 36337a05-3746-41a0-9496-26732559f3f1
loops=find_loops(ir.cfg.blocks)
